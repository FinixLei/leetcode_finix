...
void printMap(const unordered_map<string, vector<string>>& diffMap)
{
    auto it = diffMap.begin();
    for (; it != diffMap.end(); it++) {
        cout << it->first << ": " << endl;
...
// BFS: 
// 1> use queue to store each layer's words 
// 2> use set to store the visited words 
// Once find the end word, BFS ends and return the layer count. 
int ladderLength(string beginWord, string endWord, vector<string>& wordList) 
{
    ...
    while(myqueue.size()) {
        pair<string, int> tmpPair = myqueue.front();
        string tmpStr = tmpPair.first;
        int tmpLayer = tmpPair.second;
        myqueue.pop();
        
        for (string str : oneCharDiffMap[tmpStr]) {
            if (visited.find(str) == visited.end()) {
                if (str == endWord) {
                    return tmpLayer+1;
                }
                visited.insert(str);
                myqueue.push({str, tmpLayer+1});
            }
        }
    }
...
int main()
{
    ...
    int result = ladderLength(beg, end, wordList);
    ...
    result = ladderLength(beg, end, wordList);
    cout << "result is " << result << endl;
